...
export class LandmarkOverlay {
  ...
  draw(result) {
    ...
    for (let handIndex = 0; handIndex < handCount;handIndex += 1) {
      ...
      for (const landmarkIndex of landmarkIndexes) {
        const landmark =
          result.landmarks[handIndex]?.[landmarkIndex]
        ...
        const x =
          offsetX +
          landmark.x * renderedWidth

        const y =
          offsetY +
          landmark.y * renderedHeight
  ...
  clear() {
    ...
    this._context.clearRect(
      0,
      0,
      this._canvas.width / pixelRatio,
      this._canvas.height / pixelRatio
    )
  }
